...
class CorpusReader:
    ...
    def iter_data(self, display_progress=True, delim=','):
        row_iterator = csv.reader(open(self.src_filename), delimiter = delim)
        row_iterator.next() # Skip past the header.
        i = 1
...
class Datum:
    ...
    def __init__(self, row):
        ...
        for i in xrange(len(row)):
            ...
            if re.search(r"SpanList", att_name):
                row_value = row_value #self.__process_span_list(row_value)
            # Gorn lists.
            elif re.search(r"GornList", att_name):
                row_value = row_value #self.__process_gorn_list(row_value)
            # Integer-valued.
            elif att_name in ('Connective_StringPosition', 'SentenceNumber'):
                if row_value:
                    row_value = row_value#int(row_value)
                else:
                    row_value = None
            # Trees.
    ...
    def __pos(self, typ, wn_format=False, lemmatize=False):
        ...
        if lemmatize:
            results = map(self.__treebank2wn_pos, results)
            results = map(self.__lemmatize, results)
